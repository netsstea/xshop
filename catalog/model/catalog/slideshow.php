...
class ModelCatalogslideshow extends Model {
	...
	public function getslideshows($sshow) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "slideshow s LEFT JOIN " . DB_PREFIX . "slideshow_description sd ON (s.slideshow_id = sd.slideshow_id) WHERE sd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND s.sshow = '" . $sshow . "' ORDER BY s.sort_order ASC");
