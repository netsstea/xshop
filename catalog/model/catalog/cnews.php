...
class ModelCatalogCnews extends Model {
	...
	public function getCnewsToCshow($cshow, $limit) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cnews c LEFT JOIN " . DB_PREFIX . "cnews_description cd ON (c.cnews_id = cd.cnews_id) WHERE cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c.cshow = '" . $cshow . "' ORDER BY c.sort_order LIMIT " . (int)$limit);
