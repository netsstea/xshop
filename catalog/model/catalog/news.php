...
class ModelCatalogNews extends Model {
	...
	public function tinmoinhat($limit) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON (n.news_id = nd.news_id) WHERE n.news_id != '" . (int)$news_id . "' AND nd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY n.news_id DESC LIMIT " . (int)$limit);
	
		return $query->rows;
	}
	
	public function newstokey($key, $start = 0, $limit = 20) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON (n.news_id = nd.news_id) WHERE nd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY " . $key . " DESC LIMIT " . (int)$start . "," . (int)$limit);
	
		return $query->rows;
	}
	
	public function quantam($start = 0, $limit = 20) {
		if (isset($this->request->get['news_id'])) {
			$news_id = $this->request->get['news_id'];
		} else {
			$news_id = 0;
		}
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON (n.news_id = nd.news_id) WHERE nd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND n.sort_order = '100'" . " AND n.news_id != '" .(int)$news_id ."' ORDER BY n.news_id DESC LIMIT " . (int)$start . "," . (int)$limit);
	...
	public function getnewsbytag($keyword, $start = 0, $limit = 20) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON (n.news_id = nd.news_id) WHERE nd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND nd.tags LIKE '%" . $this->db->escape($keyword) . "%' ORDER BY n.news_id DESC " . " LIMIT " . (int)$start . "," . (int)$limit);
	
		return $query->rows;
	}
	
	public function gettotalnewstag($keyword) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "news n LEFT JOIN " . DB_PREFIX . "news_description nd ON (n.news_id = nd.news_id) WHERE nd.tags LIKE '%". $this->db->escape($keyword) ."%'");
