...
class ModelCatalogProduct extends Model {
	...
	public function getProductsByCategoryId($category_id, $manufacturer_id = 0, $plid = '', $sort = 'p.viewed', $order = 'DESC', $start = 0, $limit = 20) {
		$sql = "SELECT *, pd.name AS name, p.viewed, p.price,";
		
		$sql .= " p.image, ss.name AS stock, (SELECT AVG(r.rating) FROM " . DB_PREFIX . "review r WHERE p.product_id = r.product_id GROUP BY r.product_id) AS rating FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "stock_status ss ON (p.stock_status_id = ss.stock_status_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id)";
		
		if ($plid) {
			$sql .= " LEFT JOIN" . DB_PREFIX . " product_search_phanloai psp ON (psp.product_id = p.product_id)";
		}

		$sql .= " WHERE p.status = '1' AND p.date_available <= NOW() AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.category_id = '" . (int)$category_id . "'";
		...
		if($plid) {
			...
			for ($i = 0; $i < sizeof($plids); $i++) {
				$pl = '';
				if($plids[$i] < 10) {
					$pl = '00' . $plids[$i];
				} elseif($plids[$i] < 100 && $plids[$i] >= 10) {
					$pl = '0' . $plids[$i];
				} else {
					$pl = $plids[$i];
				}
				
				$sql .= " AND psp.plid LIKE '%" . $this->db->escape($pl) . "%'";
			}
		}
	...
	public function getTotalProductsByCategoryId($category_id = 0, $manufacturer_id = 0, $plid = '') {
		
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product_to_category p2c LEFT JOIN " . DB_PREFIX . "product p ON (p2c.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
		
		if ($plid) {
			$sql .= " LEFT JOIN" . DB_PREFIX . " product_search_phanloai psp ON (psp.product_id = p.product_id)";
		}
		
		$sql .= " WHERE p.status = '1' AND p.date_available <= NOW() AND p2c.category_id = '" . (int)$category_id . "'";
	...
	public function getProductsRelatedByCategoryId($product_id = 0, $category_id, $start, $limit, $manufacturer_id = 0) {
		$sql = "SELECT *, pd.name AS name, p.viewed, p.image FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "stock_status ss ON (p.stock_status_id = ss.stock_status_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.category_id = '" . (int)$category_id . "' AND p.product_id != '" . (int)$product_id . "'";
		
		if($manufacturer_id) {
			$sql .= " AND p.manufacturer_id = '" . (int)$manufacturer_id . "'";
		}
		
		$sql .= " ORDER BY p.viewed LIMIT " . (int)$start . "," . (int)$limit;
	...
	public function getProductsByManufacturerId($manufacturer_id, $keyword ='', $plid='', $sort = 'p.viewed', $order = 'DESC', $start = 0, $limit = 20) {
		...
		$sql .= " WHERE p.status = '1' AND p.date_available <= NOW() AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.manufacturer_id = '" . (int)$manufacturer_id. "'";
	...
	public function getTotalProductsByManufacturerId($manufacturer_id = 0,$keyword='', $plid='') {
		...
		$sql .= " WHERE p.status = '1' AND p.date_available <= NOW() AND p.manufacturer_id = '" . (int)$manufacturer_id . "'";
	...
	public function getProductsBychomeId($chome_id, $start = 0, $limit = 20) {
		$sql = "SELECT *, pd.name AS name, p.image, ss.name AS stock, (SELECT AVG(r.rating) FROM " . DB_PREFIX . "review r WHERE p.product_id = r.product_id GROUP BY r.product_id) AS rating FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_chome p2d ON (p.product_id = p2d.product_id) LEFT JOIN " . DB_PREFIX . "stock_status ss ON (p.stock_status_id = ss.stock_status_id) WHERE p.status = '1' AND p.date_available <= NOW() AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2d.chome_id = '" . (int)$chome_id . "'";
	...
	public function getProductsByKeyword($keyword, $sort = 'p.viewed', $order = 'DESC', $start = 0, $limit = 20) {
		if ($keyword) {
			...
			$sql .= " AND (pd.name LIKE '%" . $this->db->escape($keyword) . "%' OR pd.tags LIKE '%" . $this->db->escape($keyword) . "%')";
	...
	public function getBestSellerProducts($limit, $category_id = 0) {
		...
		if (!$product_data) { 
			...
			foreach ($query->rows as $result) {
				...
				$sql .= " WHERE p.product_id = '" . (int)$result['product_id'] . "' AND p.status = '1' AND p.date_available <= NOW() AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getProductImagesToSlide($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_image WHERE product_id = '" . (int)$product_id . "' AND slide_status= '1'");

		return $query->rows;
	}
	
	public function getProductImages($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_image WHERE product_id = '" . (int)$product_id . "'  AND slide_status= '0'");
	...
	public function getProductPhukien($product_id) {
		...
		foreach ($product_phukien_query->rows as $result) {
			$product_query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.viewed, p.image FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$result['phukien_id'] . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.date_available <= NOW() AND p.status = '1'");
	...
	public function getCatStart($product_id) {
		$result = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "'");
		$category_id = 0;
		if (isset($result->rows)) {
			foreach($result->rows  as $row ) {
				...
				if($category && $category['parent_id'] == 0) {
					$category_id = $row['category_id']; break;
				}
			}
		}
		return $category_id;
