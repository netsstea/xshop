...
/*			$this->language->load('mail/account_create');
  	...
  	private function validate() {
		if ($this->config->get('config_account')) {
			...
			if ($information_info) {
    			if (!$this->request->post['agree']) {
      				$this->error['information'] = array(sprintf($this->language->get('error_agree'), $information_info['name']));
    			}
			}
		}
		
    	if ((strlen(utf8_decode($this->request->post['telephone'])) < 3) || (strlen(utf8_decode($this->request->post['telephone'])) > 32)) {
      		$this->error['telephone'] =  array($this->language->get('error_telephone'));
    	}

    	if ((strlen(utf8_decode($this->request->post['password'])) < 4) || (strlen(utf8_decode($this->request->post['password'])) > 20)) {
      		$this->error['password'] =  array($this->language->get('error_password'));
    	}

    	if ($this->request->post['confirm'] != $this->request->post['password']) {
      		$this->error['confirm_password'] =  array($this->language->get('error_confirm'));
    	}
    	...
    	if ((strlen(utf8_decode($this->request->post['customername'])) < 3) || (strlen(utf8_decode($this->request->post['customername'])) > 64)) {
      		$this->error['customername'] =  array($this->language->get('error_customername'));
    	}
