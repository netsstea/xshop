$(function(){
    ...
    var update_list_data = function ($step, $el){
        ...
        var $conceptlist = $el.closest('div').find('.concept-list .sortable-item');
        var url =$el.closest('div').find('.concept-list').attr('data-href');
        var rowId = $el.closest('div').find('.concept-list').attr('data-id');

        if($step == 'list_items_sticky'){
            var url =$el.closest('div.concept-list').attr('data-href');
            var rowId = $el.closest('div.concept-list').attr('data-id');
            var $conceptlist = $el.closest('div.concept-list').find('.sortable-item');
        }
        
        $conceptlist.each(function(i, el){
            if($(el).attr('data-sticked') === 'yes')
                stickyItemsArr.push($(el).attr('data-item-id'));               
            itemsArr.push($(el).attr('data-item-id'));
        });
        
        var rowId = $el.closest('div').attr('data-id');
        var postdata = {
    ...
    CL['callbacks']['list-items-order-search-items'] = function ($form, data, params){
        ...
        if (data.success)
        {
            ...
            /* $("#search-results").height($(window).height() - 20); */
            ...
            $("search-results").droppable({
                ...
                hoverClass: "ui-state-hover",
                drop: function(event, ui) {
                    $("search-results").append($(ui.draggable));
                }
            });
