 ...
 * @author tran
 */ 
class Source_IndexController extends Cl_Controller_Action_NodeIndex 
{
    public function init()
    {
        //$this->daoClass = "Cl_Dao_Node_Source";
        //$this->commentDaoClass = "Cl_Dao_Comment_Source";
    ...
    {
        assure_perm('source_new');
        $this->genericNew("Source_Form_New", "Dao_Node_Source", "Node");
        ...
        if(isset($this->ajaxData)) {
            ...
            if (isset($this->ajaxData['result'])) //success
            {
                ...
                else 
                {
                	...
                	$this->ajaxData['data'] = array('url' => node_link('source' , $this->ajaxData['result']));
                }
            }
        }
        Bootstrap::$pageTitle = t("new_source",1);
    }
    ...
    public function updateAction()
    {
         ...
         */
    	$this->setLayout("admin");
        $this->genericUpdate("Source_Form_Update", $this->daoClass ,"", "Node");
        Bootstrap::$pageTitle = t("update_source",1);
    }

    public function searchAction()
    {
        assure_perm("search_source");//by default
        $this->setLayout("admin");
        $this->genericSearch("Source_Form_Search", $this->daoClass, "Node");
        $this->setLayout("admin");
    ...
    public function searchCommentAction()
    {
        ...
        $this->genericSearch("Source_Form_SearchComment", $commentClass, "");
        $this->setLayout("admin");
    ...
    public function viewAction()
    {
        ...
        {
        	//$row = Dao_Node_Source::getInstance()->reformatSource($row, 'view');
    ...
    {
    	//TODO: Implement this
    	return has_perm("new_source_comment");
    }
    ...
    {
        assure_role('admin_source');
    	$ids = $this->getStrippedParam('ids');
    	$in = explode(',', $ids);
    	$where = array('id' => array('$in' => $in));
    	Dao_Node_Source::getInstance()->delete($where);
    	Dao_Node_Source::getInstance()->clearAllStaticCache();
    ...
    public function listAction()
    {
        ...
        */
        ...
        $this->genericSearch('Source_Form_Search', "Dao_Node_Source", 'Node');
        if (is_ajax())
        {
        	if (isset($this->ajaxData['result']))
        	{
        		...
        		{
        			//$row = Dao_Node_Source::getInstance()->reformatSource($row);
        			$list[$i] = $row;
        		}
        	}
        	else 
        ...
        {
            $this->setNoRenderer();
            echo $this->renderScript('widget/source-list.phtml');
        }
    }
