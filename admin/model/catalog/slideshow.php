...
class ModelCatalogslideshow extends Model {
	public function addslideshow($data) {
      	$this->db->query("INSERT INTO " . DB_PREFIX . "slideshow SET sort_order = '" . (int)$data['sort_order'] . "', link = '" . $this->db->escape($data['link']) . "', sshow = '" . $this->db->escape($data['sshow']) . "'");
		...
		if (isset($data['image'])) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "slideshow SET image = '" . $first_img . "' WHERE slideshow_id = '" . (int)$slideshow_id . "'");
		}
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "history SET user_id = '" . (int)$this->user->getId() . "', username = '" . $this->db->escape($this->user->getUserName()) . "', generic = 'slideshow_add', code = 'slideshow_id=" . (int)$slideshow_id . "', date_added = NOW()");
		
		$this->cache->delete('slideshow');
	}
	
	public function editslideshow($slideshow_id, $data) {
      	$this->db->query("UPDATE " . DB_PREFIX . "slideshow SET sort_order = '" . (int)$data['sort_order'] . "', link = '" . $this->db->escape($data['link']) . "', sshow = '" . $this->db->escape($data['sshow']) . "' WHERE slideshow_id = '" . (int)$slideshow_id . "'");
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "history SET user_id = '" . (int)$this->user->getId() . "', username = '" . $this->db->escape($this->user->getUserName()) . "', generic = 'slideshow_edit', code = 'slideshow_id=" . (int)$slideshow_id . "', date_added = NOW()");
