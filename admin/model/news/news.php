...
class ModelNewsNews extends Model {
	public function addnews($data) {
		...
		foreach ($data['news_description'] as $language_id => $value) {
			...
			$description = $this->model_tool_downloadimage->downloadImage($value['description'],$news_id, 'news', false);

			$this->db->query("INSERT INTO " . DB_PREFIX . "news_description SET news_id = '" . (int)$news_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', tags = '" . $this->db->escape($value['tags']) . "', description = '" . $this->db->escape($description) . "'");
		}
		
		if ($data['image']) {
			$this->db->query("UPDATE " . DB_PREFIX . "news SET image = '" . $this->db->escape($data['image']) . "' WHERE news_id = '" . (int)$news_id . "'");
		} else {
			$this->db->query("UPDATE " . DB_PREFIX . "news SET image = '" . $this->db->escape($this->model_tool_downloadimage->getUrlImage($description)) . "' WHERE news_id = '" . (int)$news_id . "'");
		}
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "history SET user_id = '" . (int)$this->user->getId() . "', username = '" . $this->db->escape($this->user->getUserName()) . "', generic = 'news_add', code = 'news_id=" . (int)$news_id . "', date_added = NOW()");
	...
	public function editnews($news_id, $data) {
		...
		foreach ($data['news_description'] as $language_id => $value) {
			...
			if($news_info) { $date_added = $news_info['date_added']; } else { $date_added = false; }
			
			$description = $this->model_tool_downloadimage->downloadImage($value['description'],$news_id, 'news', $date_added);
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "history SET user_id = '" . (int)$this->user->getId() . "', username = '" . $this->db->escape($this->user->getUserName()) . "', generic = 'news_edit', code = 'news_id=" . (int)$news_id . "', date_added = NOW()");
