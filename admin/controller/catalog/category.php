...
class ControllerCatalogCategory extends Controller { 
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach ($this->request->post['selected'] as $category_id) {
				...
				$list = $list . '_' . $category_id;
			}
			
			$this->model_user_history->addHistory('category_delete','category_id=' . $list);
	...
	private function getForm() {
    	...
    	$this->data['manufacturers'] = $this->model_catalog_manufacturer->getManufacturers();
 		...
 		if (isset($this->request->post['category_manufacturer'])) {
			$this->data['category_manufacturers'] = $this->request->post['category_manufacturer'];
		} elseif (isset($category_info)) {
			$this->data['category_manufacturers'] = $this->model_catalog_category->getCategoryManufacturer($this->request->get['category_id']);
		} else {
		...
		if (isset($this->request->post['banner'])) {
			...
			$this->data['banner'] = $category_info['banner'];
		} else {
			$this->data['banner'] = '';
		}

		if (isset($category_info) && $category_info['banner'] && file_exists(DIR_IMAGE . $category_info['banner'])) {
			$this->data['preview_banner'] = image_resize($category_info['banner'], 100, 100);
		} else {
		...
		if (isset($this->request->post['icon_menu'])) {
			...
			$this->data['icon_menu'] = $category_info['icon_menu'];
		} else {
			$this->data['icon_menu'] = '';
		}

		if (isset($category_info) && $category_info['icon_menu'] && file_exists(DIR_IMAGE . $category_info['icon_menu'])) {
			$this->data['preview_icon_menu'] = image_resize($category_info['icon_menu'], 100, 100);
		} else {
			$this->data['preview_icon_menu'] = image_resize('no_image.jpg', 100, 100);
		}
		
		if (isset($this->request->post['icon_mobile'])) {
			$this->data['icon_mobile'] = $this->request->post['icon_mobile'];
		} elseif (isset($category_info)) {
			$this->data['icon_mobile'] = $category_info['icon_mobile'];
		} else {
			$this->data['icon_mobile'] = '';
		}

		if (isset($category_info) && $category_info['icon_mobile'] && file_exists(DIR_IMAGE . $category_info['icon_mobile'])) {
			$this->data['preview_icon_mobile'] = image_resize($category_info['icon_mobile'], 100, 100);
		} else {
			$this->data['preview_icon_mobile'] = image_resize('no_image.jpg', 100, 100);
		}
		...
		$this->data['phanloais'] = $this->model_catalog_phanloai->getPhanloais1(0);
		
		if (isset($this->request->post['cshow'])) {
			$this->data['cshow'] = $this->request->post['cshow'];
		} elseif (isset($category_info)) {
			$this->data['cshow'] = explode(',', $category_info['cshow']);
		} else {
			$this->data['cshow'] = array();
		}
