...
class ControllerCatalogInformation extends Controller { 
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach ($this->request->post['selected'] as $information_id) {
				...
				$list = $list . '_' . $information_id;
			}
			
			$this->model_user_history->addHistory('information_delete','information_id=' . $list);
	...
	private function getList() {
		...
		$this->load->model('tool/info');
		
    	foreach ($results as $result) {
		
			$cinformation_description = $this->model_catalog_cinformation->getcinformationDescriptions($result['cinformation_id']);
			
			if($cinformation_description) {
				$cinformation_description_name = $cinformation_description[$result['language_id']]['name'];
			} else {
				$cinformation_description_name = '';
			}
			
			$cinformation = $this->model_catalog_cinformation->getcinformation($result['cinformation_id']);
			...
			if($cinformation && $this->model_tool_info->infotocshow($cinformation['cshow'])) {
				$cshow = $this->model_tool_info->infotocshow($cinformation['cshow']);
			} else {
				$cshow = '';
			}
			...
			$this->data['informations'][] = array(
				...
				'cinformation_name' => $cinformation_description_name,
				'cinformation_cshow' => $cshow,
				'cinformation_href' => $this->url->https('catalog/cinformation/update&cinformation_id=' . $result['cinformation_id']),
	...
	private function getForm() {
		...
		if (isset($information_info) && $information_info['image'] && file_exists(DIR_IMAGE . $information_info['image'])) {
			$this->data['preview'] = image_resize($information_info['image'], 100, 100);
		} else {
